...
decode kind=spi clk=4 mosi=5 miso=6 cs=7 cpol=0 cpha=0 order=msb bits=8</code></pre>
  ...
  <p><code>gen pin=&lt;gpio&gt;</code> drives <b>any GPIO</b>. Jumper a spare pin (G47 on M5-Bus 23, say) to a probe channel and the test then covers the <b>external</b> path as well — pad, real wire, pad. Up to four pins at once.</p>
  <pre><code>&gt; gen ch=0 freq=1000000 duty=50
...
},
{
  ...
  <p>A soft delimiter cannot express "never end a frame": <code>eof_data_len</code> rejects zero and the register is 16 bits, so a boundary lands at least every 65535 samples even though an infinite transaction has no use for one. A level delimiter does accept <code>eof_data_len = 0</code>, meaning "run until the enable goes inactive", so <b>G51 is tied permanently active as that enable</b> and no frame ever ends. The chip drives the pin itself, so nothing has to be wired to it, but <b>it cannot be used for anything else</b>. G51 is the pick because the schematic gives it a dedicated TVS instead of one of the shared ESD arrays every other M5-Bus pin sits on, so a permanently driven signal is not parked next to a probe channel.</p>
  <table>
